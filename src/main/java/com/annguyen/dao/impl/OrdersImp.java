...
import com.annguyen.dao.OrdersDAO;
...
//		
//		return order;
		return session.get(Orders.class, order_id);
	}
	
	public Orders getOrderByName(String name) {
		// TODO Auto-generated method stub
		Session session = sessionFactory.getCurrentSession();
		Orders order = (Orders) session.createQuery("from Orders where customer_name = '"+name+"'").getSingleResult();
